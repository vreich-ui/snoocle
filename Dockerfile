...
#
# GOOGLE_CLOUD_PROJECT and SNOOCLE_MCP_TRUST_PROXY are baked into the IMAGE (not
# left to the deploy command) so they survive automated redeploys — a Cloud
# Build trigger that rebuilds and redeploys can't accidentally drop them. A
# Cloud Run service-config env var of the same name still overrides the baked
# value at deploy time, so this is a durable default, not a lock-in.
#   * GOOGLE_CLOUD_PROJECT: the fixed GCP project this image is deployed to
#     (Firestore uses it via Application Default Credentials). Override with a
#     --build-arg-driven ENV or a deploy-time env var to target another project.
